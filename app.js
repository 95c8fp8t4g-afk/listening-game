...
// ===== v40: speech retry without stacking points =====
// Speech attempts are previews only. Score is committed exactly once,
// when the student presses NEXT after choosing the final attempt.

function speech(q){
 chrome(`<div class="round">${esc(q.round||"Question")}</div>
   <div class="prompt">${esc(q.title||"문장을 읽어 보세요.")}</div>
   <div class="speak-target">${esc(q.target||"")}</div>
   <div id="speechResult" class="speech-result"><p class="note">마이크 버튼을 눌러 문장을 읽어 보세요.</p></div>
   <div class="actions">
     <button class="btn secondary" id="mic">🎙️ 음성 인식 시작</button>
     <button class="btn" id="scoreSpeechV40" disabled>점수 받기</button>
   </div>`);
 S.transcript="";
 S.speechScore=null;
 S.finalSpeechAttemptV40=null;
 $("#mic").onclick=()=>listen(q);
 $("#scoreSpeechV40").onclick=()=>previewSpeechScoreV40(q);
}

function previewSpeechScoreV40(q){
 if(!S.transcript)return alert("먼저 음성 인식을 해 주세요.");
 const raw=similarity(S.transcript,q.target);
 const sb=speedBonus();
 const gain=Math.round(raw*0.6)+(raw>=70?Math.round(sb.points*0.5):0);

 // Replace the previous attempt; never touch S.score here.
 S.speechScore=raw;
 S.finalSpeechAttemptV40={
   raw,
   gain,
 ...
 if(result)result.innerHTML=`
   <div class="feedback ${raw>=70?"ok":"bad"}">
     현재 발음 점수 <b>${raw}점</b> · 게임 점수 <b>+${gain}점</b><br>
     <small>다시 녹음하면 이 점수는 새 점수로 교체됩니다.</small>
   </div>`;

 const actions=document.querySelector(".card .actions");
 if(actions)actions.innerHTML=`
   <button class="btn secondary" id="retrySpeechV40">🎙️ 다시 하기</button>
   <button class="btn" id="nextSpeechV40">이 점수로 NEXT →</button>`;

 $("#retrySpeechV40").onclick=()=>{
   // Clear only this attempt. No game score has been awarded yet.
   S.transcript="";
   S.speechScore=null;
   S.finalSpeechAttemptV40=null;
   if(result)result.innerHTML=`<p class="note">다시 음성 인식을 해 주세요.</p>`;
   const actions=document.querySelector(".card .actions");
   actions.innerHTML=`<button class="btn secondary" id="mic">🎙️ 음성 인식 시작</button><button class="btn" id="scoreSpeechV40" disabled>점수 받기</button>`;
   $("#mic").onclick=()=>listen(q);
   $("#scoreSpeechV40").onclick=()=>previewSpeechScoreV40(q);
 };

 $("#nextSpeechV40").onclick=()=>commitSpeechAndNextV40();
}

function commitSpeechAndNextV40(){
 const a=S.finalSpeechAttemptV40;
 if(!a)return alert("먼저 음성 점수를 받아 주세요.");

 // Lock immediately so double-taps cannot award twice.
 const btn=$("#nextSpeechV40");
 ...
 S.answers.push({
   q:S.q+1,type:"speak",correct:raw>=70,response:a.transcript,
   speechScore:raw,points:gain,speedBonus:a.speedBonus
 });
 S.finalSpeechAttemptV40=null;
 updateLiveSession();
 ...
 if(after>before)evolutionPopup(after);
 next();
}

// Old finishSpeech must not award anything if an older button somehow calls it.
function finishSpeech(){
 const q=S.playQuestions?.[S.q];
 if(q?.type==="speak")return previewSpeechScoreV40(q);
}

// Enable score button after speech recognition produces a transcript.
// This observes the existing result changes without altering recognition logic.
const _listenV40=listen;
listen=function(q){
 const result=_listenV40.apply(this,arguments);
 let tries=0;
 const timer=setInterval(()=>{
   tries++;
   const btn=$("#scoreSpeechV40");
   if(btn && S.transcript){btn.disabled=false;clearInterval(timer);}
